...
`app.metadataCache.getFileCache(file)` can still return the PRE-write frontmatter for a beat after `app.fileManager.processFrontMatter()`'s own promise resolves (confirmed via direct repro: write, then immediate read-back with zero manual delay, returned the stale value) — its re-index runs on a separate, unawaited pass. Fixed at the seam, not per call site: every write in `vault/writer.ts` goes through its `writeFrontmatter()` helper, which doesn't resolve until `metadataCache`'s own `"changed"` event fires for that file (falling back to a timeout so a missed event can't hang a write forever). A caller doing `scanVault()`/`reload()` right after any `vault/writer.ts` write can trust it reflects the write — no manual in-memory patching needed. Testing this: `vault/fixtures/fake-app.ts`'s `deferIndexing` option holds `getFileCache` stale until a test explicitly calls `flushMetadataCache(path)`, so the wait/timeout behavior is exercisable without real timers (see `writer.test.ts`'s `describe("writeFrontmatter", ...)`).
...
`src/vault/fixtures/fake-app.ts` — minimal in-memory Obsidian `App` fake (just the `vault`/`metadataCache`/`fileManager` subset `reader.ts`/`writer.ts` call) for testing vault code without a DOM/jsdom harness; supports injecting a write failure mid-batch via a `failOn` predicate, and simulating `metadataCache` re-index lag via `deferIndexing` + a manual `flushMetadataCache(path)` call (see `writer.test.ts`'s `writeFrontmatter` tests). Reuse for future vault-write characterization tests rather than rebuilding.

`obsidian` is a types-only npm package (`"main": ""`) — `Setting`/`PluginSettingTab`/`Notice` have no runtime implementation, so a class built on them (e.g. `settings-tab.ts`, `settings-*-section.ts`) can't be instantiated in a vitest test without building fake UI shims. When such a class needs test coverage, extract the Obsidian-free decision logic into its own plain class/function and test that instead (see `SettingsDirtyTracker` in `settings-dirty-tracker.ts`, and `saveOrder` in `settings-context.ts`, both tested with zero DOM/Obsidian imports).
